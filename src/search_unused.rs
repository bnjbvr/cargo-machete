...
fn get_full_manifest(
    dir_path: &Path,
    manifest_path: &Path,
) -> anyhow::Result<(cargo_toml::Manifest<PackageMetadata>, meta::MetadataFields)> {
    ...
    let mut workspace_metadata = MetadataFields {
        ignored: vec![],
    ...
    while dir_path.pop() {
            ...
            cargo_toml::Manifest::<PackageMetadata>::from_path_with_metadata(&workspace_cargo_path)
        {
            if let Some(workspace) = &workspace_manifest.workspace {
                // Look for `workspace.metadata.cargo-machete` in the workspace Cargo.toml.
                if let Some(metadata) = workspace
                    .metadata
                    .as_ref()
                    .and_then(|metadata| metadata.cargo_machete.as_ref())
                {
                    workspace_metadata.clone_from(metadata);
                }
    ...
    Ok((manifest, workspace_metadata))
}
...
pub(crate) fn find_unused(
    ...
    let (manifest, mut workspace_metadata) = get_full_manifest(&dir_path, manifest_path)?;
    ...
    let workspace_ignored: HashSet<_> = std::mem::take(&mut workspace_metadata.ignored)
        .into_iter()
        .collect();
    ...
    let results: Vec<SingleDepResult> = dependencies
        ...
        .filter_map(|(dep_name, crate_name)| {
            // If the crate was renamed, use the renamed name.
            let crate_name = renamed
                .get(dep_name.as_str())
                .or_else(|| workspace_metadata.renamed.get(dep_name.as_str()))
                .map(Box::as_ref)
