...
fn collect_paths(path: &Path, options: CollectPathOptions) -> Result<Vec<PathBuf>, ignore::Error> {
    ...
    paths.retain(|manifest_path| {
        if crate::search_unused::is_package_ignored_by_parent(manifest_path).unwrap_or(false) {
            log::debug!(
...
fn run_machete() -> anyhow::Result<bool> {
    ...
    let args: MacheteArgs = if running_as_cargo_cmd() {
        argh::cargo_from_env()
    } else {
        argh::from_env()
    };

    run_machete_with_args(args)
}

fn run_machete_with_args(mut args: MacheteArgs) -> anyhow::Result<bool> {
    if args.version {
...
#[cfg(test)]
#[test_log::test]
fn test_ignore_dirs_workspace_nested_deep_works() {
    let path: PathBuf = "./integration-tests/ignored-dirs-workspace-nested-deep".into();
